...
class BroadcastPushViewController: BasePushViewController, UITableViewDelegate, UITableViewDataSource, UITextViewDelegate, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    let titleTextFieldHeight:CGFloat = 50
    ...
    let maxTitleCount = 20
    let maxContentCount = 400
    
    let rowHeight : [CGFloat] = [60, 200 + 10 + 10 + 18, 35, UIDevice.SCREEN_WIDTH + 30]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if(indexPath.row == 0){
            ...
            titleTextField = UITextField(frame: CGRect(x: padding, y: padding, width: ScreenWidth - padding*2, height: titleTextFieldHeight))
            titleTextField.placeholder = "标题(最多20字)"
            titleTextField.font = .boldSystemFont(ofSize: titleFontSize)
...
//            cell.backgroundColor = .secondarySystemBackground
            ...
            separater.backgroundColor = .systemGray
            separater.alpha = 0.3
            cell.addSubview(separater)
...
extension BroadcastPushViewController{
